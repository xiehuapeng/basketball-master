...
这个项目已经从“几个本地实验脚本”升级成了“可运行的 MVP 工程骨架”，并且单视频分析、球星对比分析、FastAPI 服务、本地篮球检测和上传任务链路都已实现。当前状态基线为 2026-08-04。

当前最重要的判断：
- 工程化基础已经具备
- 当前主能力建立在 `MediaPipe Pose + 本地 YOLO 篮球检测 + 时序启发式规则` 上
...
当前检测已完成本地优先、远端备用的可插拔接入：
- 支持 `local`（Ultralytics YOLO）
- 支持 `roboflow`
- 支持 `huggingface` / `hf`
- 支持离手附近抽帧调用
- 支持必要时扩大向前回溯窗口
...
- 已补充单元测试覆盖关键修正逻辑
- 已增加主体人物时序锁定、远景小人物自适应和多信号主手判断
- 已增加静止背景球过滤、飞行轨迹证据和离手帧双向修正
...
- 篮球微调模型在部分远景会把手误检为球，仍需候选轨迹过滤
- 主体锁定是基于髋中心和尺度连续性的过滤，不是完整多人跟踪器
- 长视频多次投篮尚不能自动切片

### 产品问题

- 当前只有内嵌 MVP 上传页，尚未形成独立产品前端
- 当前任务队列和状态存于内存，重启会丢失
- 还没有训练报告沉淀和历史趋势
- 还没有球星模板管理
...
- 上传大小、用户隔离、任务持久化和服务并发策略尚未产品化
- 远端推理仍存在 Hosted API 超时风险，需要后续补容错和重试
- 还没有完整日志、监控和错误码分层
